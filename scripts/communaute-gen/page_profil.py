  ...
  <div class="cm-split">
    ...
    <div style="display: flex; flex-direction: column; gap: 24px;">
      <section class="cm-card">
        ...
        <sc-if value="{{ noReports }}"><div class="cm-empty">Aucun bug signalé. Depuis l'application ou le forum, vos signalements apparaîtront ici avec leur état.</div></sc-if>
        <ul class="cm-list">
          <sc-for list="{{ reports }}" as="b">
            <li class="cm-row">
              <div class="cm-row-main">
                <p class="cm-row-title" style="font-size: 14px;">{{ b.title }}</p>
                <div class="cm-row-meta"><span>{{ b.date }}</span><span>{{ b.severity }}</span></div>
              </div>
...
class Component extends DCLogic {
  ...
  renderVals() {
    ...
    const reports = (s.reports || []).map((b) => {
      const st = STATES[b.state] || STATES.new;
      return { title: b.title, date: RDB.fmtDate(b.created_at), severity: SEV[b.severity] || 'gênant', state: st[0], cls: st[1] };
    });
